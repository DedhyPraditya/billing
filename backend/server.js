...
async function jalankanMigration() {
    ...
    const migrations = [
        ...
        {
            cek:  `SELECT COUNT(*) AS n FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='reseller' AND COLUMN_NAME='alamat'`,
            sql:  `ALTER TABLE reseller ADD COLUMN alamat VARCHAR(255) NULL AFTER no_hp`,
            nama: 'reseller.alamat'
        }
    ];
...
require('./services/olt-cron').start({ hour: 2, minute: 30 });
require('./services/olt-alert').start();
require('./services/acs-alert').start();   // monitor GenieACS: RX memburuk + ONU offline
...
// Restore backup bisa berisi dump SQL besar → parser khusus limit lebih tinggi,
// dipasang SEBELUM parser global. Begitu ini mem-parse body, parser global
// otomatis dilewati (body-parser men-set req._body). Endpoint lain tetap 12mb.
// CATATAN: dump sangat besar (>200MB) sebaiknya di-restore via MySQL CLI di
// server (mysql ... < backup.sql) untuk hindari pemakaian memori berlebih.
...
app.get('/', (req, res) => {
    ...
    res.sendFile(path.join(__dirname, '../frontend/index.html'), { cacheControl: false });
});

// --- Halaman admin ---
app.get('/admin', (req, res) => {
    res.set('Cache-Control', 'no-cache, must-revalidate');
    res.sendFile(path.join(__dirname, '../frontend/admin.html'), { cacheControl: false });
});

app.get('/client', (req, res) => {
    res.set('Cache-Control', 'no-cache, must-revalidate');
    res.sendFile(path.join(__dirname, '../frontend/client.html'), { cacheControl: false });
});
...
app.use('/api/kabel',     require('./routes/kabel'));
